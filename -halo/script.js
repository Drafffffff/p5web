...
function setup() {
    ...
    for (let i = 0; i < n; i++) {
        ...
        let a = {
            x: (width / 2) + 100 * sin(map(i, 0, n - 1, 0, TAU)),
            y: (height / 2) + 100 * cos(map(i, 0, n - 1, 0, TAU))
        }
        poly.push(a)
    }
...
function drawPoly(dx, dy) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        vertex(poly[i].x + dx / logMap(bias, width, 0, dx, 45) + g, poly[i].y + dy / logMap(bias, height, 0, dy, 45) + g)
    }
    endShape()
}
